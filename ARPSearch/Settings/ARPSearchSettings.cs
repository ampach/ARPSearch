...
namespace ARPSearch.Settings
{
    public class ARPSearchSettings : IARPSearchSettings
    {
        public ARPSearchSettings()
        {
            ...
            this.ResultsMapping = new Dictionary<Guid, string>();
        }

        //public Guid RootSitecoreItem { get; private set; }
        public string SearchServiceAjaxRequestUrl { get; private set; }

        public Dictionary<Guid, string> Facets { get; private set; }
        public Dictionary<Guid, string> ResultsMapping { get; private set; }
        ...
        public void AddResultMapping(string key, System.Xml.XmlNode node)
        {
            AddFacetDef(node);
        }
        ...
        public void AddResultMapping(System.Xml.XmlNode node)
        {
            var guid = Sitecore.Xml.XmlUtil.GetAttribute("resultTemplateId", node);
            var name = Sitecore.Xml.XmlUtil.GetChildValue("templateId", node);
            this.ResultsMapping.Add(new Guid(guid), name);
        }
